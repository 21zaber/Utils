...
silent only
cd ~/work/ansocial/docker/aerospike-server/conf.small
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +1 aerospike.conf
badd +1 ae
badd +0 aerospike.backup-all.conf
argglobal
silent! argdel *
argadd aerospike.conf
edit aerospike.conf
...
let s:l = 133 - ((0 * winheight(0) + 35) / 71)
...
wincmd w
argglobal
edit aerospike.backup-all.conf
setlocal keymap=
setlocal noarabic
setlocal noautoindent
setlocal backupcopy=
...
setlocal wrap
setlocal wrapmargin=0
let s:l = 232 - ((0 * winheight(0) + 35) / 71)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
232
